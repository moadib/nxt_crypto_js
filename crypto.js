...
function add(r, a, b)
{
    ...
    for (var i = 1; i <= 14; i++)
        r[i] = (v = Math.floor(v / 0x10000) + a[i] + b[i]) % 0x10000;
...
function sub(r, a, b)
{
    ...
    for (var i = 1; i <= 14; i++)
        r[i] = (v = Math.floor(v / 0x10000) + 0x7fff8 + a[i] - b[i]) % 0x10000;
...
function mul(r, a, b) {
    ...
    r[0] = (v = 0x800000 + z[0] + (y[8] -x[8] -z[8] + x[0] -0x80) * 38) % 0x10000;
    for (var i = 9; i <= 15; i++)
        r[i-8] = (v = 0x7fff80 + Math.floor(v / 0x10000) + z[i-8] + (y[i] -x[i] -z[i] + x[i-8]) * 38) % 0x10000;
    for (var i = 0; i <= 6; i++)
        r[i+8] = (v = 0x7fff80 + Math.floor(v / 0x10000) + z[i+8] + y[i] -x[i] -z[i] + x[i+8] * 38) % 0x10000;
    r[15] = 0x7fff80 + Math.floor(v / 0x10000) + z[15] + y[7] -x[7] -z[7] + x[15] * 38;
    reduce(r);
...
function mul_small(r, a, b)
{
    ...
    r[0] = (v = a[0] * b) % 0x10000;
    for (var i = 1; i <= 14; i++)
        r[i] = (v = Math.floor(v / 0x10000) + a[i]*b) % 0x10000;
